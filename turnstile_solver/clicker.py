...
try:
    ...
    # PyAutoGUI not available (e.g. no X server or module missing).
    # This is expected in headless/docker environments where we use CDP.
    pyautogui = None
...
class TurnstileClicker:
    ...
    def __init__(self, driver, method: Literal["cdp", "pyautogui"]):
        ...
        if self.method == "pyautogui" and pyautogui is None:
            print("Warning: PyAutoGUI is not available (missing DISPLAY or module). Fallback to CDP.")
            self.method = "cdp"
